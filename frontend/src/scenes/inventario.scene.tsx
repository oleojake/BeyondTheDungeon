...
function EquipSlot({
  ...
  return (
    <div
      ...
      onClick={onOpen}
      title={t(`scenes.inventoryScene.slots.${slot.key}`)}
      className={[
      ...
      {item ? (
        <>
          <span className="text-[8px] font-bold tracking-widest text-amber-400/90 uppercase text-center leading-none px-1">
            {t(`scenes.inventoryScene.slots.${slot.key}`)}
          </span>
        ...
        <>
          ...
          <span className="text-[8px] text-amber-600/60 group-hover:text-amber-500/80 text-center leading-tight px-1 transition-colors">
            {t("scenes.inventoryScene.common.empty")}
          </span>
        </>
      )}
    </div>
      ...
      <div className="bg-[#1e0d05] border border-amber-800/50 rounded-xl shadow-2xl w-full max-w-md mx-4 flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-amber-900/40">
          <h3 className="text-amber-300 font-semibold text-sm">
            {t("scenes.inventoryScene.slotPicker.equip")} - <span className="text-amber-500">{slotLabel}</span>
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-3 border-b border-amber-900/30">
          <div className="flex gap-2">
            <input
              ...
              placeholder={t("scenes.inventoryScene.slotPicker.searchPlaceholder", { slot: slotLabel.toLowerCase() })}
            ...
            <button
              ...
              title={t("scenes.inventoryScene.slotPicker.addCustom")}
          ...
          <div className="flex items-center justify-between mt-2">
            <label className="flex items-center gap-1.5 text-xs text-amber-500/80 cursor-pointer select-none">
              ...
              />
              {t("scenes.inventoryScene.slotPicker.searchAll", { count: allItems.length })}
            </label>
            <span className="text-[10px] text-amber-600/60">
              {query.trim()
                ? t("scenes.inventoryScene.slotPicker.results", { count: suggestions.length })
                : t("scenes.inventoryScene.slotPicker.objects", { count: pool.length })}
            </span>
          </div>
        </div>
        {/* List */}
        <div className="overflow-y-auto flex-1 divide-y divide-amber-900/20">
          {allItems.length === 0 ? (
            <p className="px-4 py-6 text-xs text-amber-600 italic text-center">
              {t("scenes.inventoryScene.common.loadingCompendium")}
            </p>
          ) : suggestions.length === 0 ? (
            <p className="px-4 py-4 text-xs text-amber-600 italic text-center">
              {t("scenes.inventoryScene.slotPicker.noResults")}
            </p>
          ) : (
          ...
          {allItems.length > 0 && (
            <div className="px-3 py-1 border-b border-amber-900/40 flex items-center justify-between">
              <span className="text-[10px] text-amber-600/70">
                {query.trim()
                  ? t("scenes.inventoryScene.autocomplete.results", { count: suggestions.length })
                  : t("scenes.inventoryScene.autocomplete.objectsFiltered", { count: pool.length })}
              </span>
            </div>
          )}
          <div className="max-h-56 overflow-y-auto">
            {allItems.length === 0 ? (
              ...
              <p className="px-3 py-2 text-xs text-amber-600 italic">
                {t("scenes.inventoryScene.autocomplete.noResults")}
              </p>
            ) : (
              suggestions.map((item) => {
                const dropTags = getItemTags(item.stats ?? {});
            ...
            <div className="sticky bottom-0 border-t border-amber-900/50 px-3 py-1.5 bg-[#1e0e05]">
              <label className="flex items-center gap-2 text-xs text-amber-500/80 cursor-pointer select-none">
                <input
                  ...
                  className="accent-amber-600 w-3 h-3"
                />
                {t("scenes.inventoryScene.slotPicker.searchAllWithObjects", { count: allItems.length })}
              </label>
            </div>
          )}
        </div>
...
export const InventarioScene = () => {
  ...
  const CURRENCY_LABELS: {
    ...
    { key: "pp", color: "text-slate-300" },
    { key: "po", color: "text-yellow-400" },
    { key: "pe", color: "text-teal-400" },
    { key: "pa", color: "text-gray-300" },
    { key: "pc", color: "text-orange-400" },
  ];
        ...
        <section className="rounded-2xl bg-gradient-to-r from-amber-600/30 via-yellow-500/20 to-amber-600/30 p-6 shadow-xl border border-amber-600/20">
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold text-amber-50">{t("scenes.inventoryScene.header.title")}</h1>
              </div>
              <p className="text-sm text-amber-100/90">
                {t("scenes.inventoryScene.header.subtitle")}
              </p>
            </div>
            <Button
              onClick={handleSave}
            ...
            >
              ...
              {saveStatus === "saving"
                ? t("scenes.inventoryScene.save.saving")
                : saveStatus === "saved"
                  ? t("scenes.inventoryScene.save.saved")
                  : saveStatus === "error"
                    ? t("scenes.inventoryScene.save.error")
                    : !user
                      ? t("scenes.inventoryScene.save.loginToSave")
                      : !linkedCharId
                        ? t("scenes.inventoryScene.save.chooseCharacter")
                        : t("common.save")}
            </Button>
          </div>
        </section>
          ...
          {!user && (
            <div className="flex items-start gap-3 rounded-lg border border-amber-700/40 bg-amber-950/40 px-4 py-3 text-sm text-amber-300">
              ...
              <span>
                {t("scenes.inventoryScene.guestBanner.textBefore")} {" "}
                <Link
                  to={routes.login}
                  className="underline hover:text-amber-100"
                >
                  {t("scenes.inventoryScene.guestBanner.login")}
                </Link>{" "}
                {t("scenes.inventoryScene.guestBanner.or")}{" "}
                <Link
                  to={routes.register}
                  className="underline hover:text-amber-100"
                >
                  {t("scenes.inventoryScene.guestBanner.register")}
                </Link>
                .
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 rounded-lg border border-amber-800/40 bg-amber-950/30 px-4 py-3 text-sm">
              ...
              {charsLoading ? (
                <span className="text-amber-500 flex items-center gap-2">
                  ...
                  {t("scenes.inventoryScene.characterSelector.loading")}
                </span>
              ) : characters.length === 0 ? (
                <span className="text-amber-600">
                  {t("scenes.inventoryScene.characterSelector.noCharacters")} {" "}
                  <Link
                    to={routes.misFichas}
                    className="underline hover:text-amber-400"
                  >
                    {t("scenes.inventoryScene.characterSelector.createFirst")}
                  </Link>
                  .
                </span>
              ) : linkedCharId ? (
                <div className="flex items-center gap-3 flex-1 flex-wrap">
                  <span className="text-amber-300">
                    {t("scenes.inventoryScene.characterSelector.linkedTo")} {" "}
                  ...
                  >
                    {t("scenes.inventoryScene.characterSelector.change")}
                  </button>
                </div>
              ) : (
                <div className="flex items-center gap-2 flex-1 flex-wrap">
                  <span className="text-amber-500 shrink-0">
                    {t("scenes.inventoryScene.characterSelector.choose")}
                  </span>
                  <select
                    defaultValue=""
                    onChange={(e) => {
                      ...
                      if (hasLocal) {
                        const load = window.confirm(
                          t("scenes.inventoryScene.characterSelector.loadConfirm", { name: char.name }),
                        );
                  ...
                  >
                    <option value="" disabled>
                      {t("scenes.inventoryScene.characterSelector.selectPlaceholder")}
                    </option>
                    {characters.map((c) => (
            ...
            <div className="flex items-center gap-6 flex-wrap flex-1">
              <div className="text-center">
                <div className="flex items-center gap-2 justify-center mb-0.5">
                  <p className="text-[10px] uppercase tracking-widest text-amber-600">
                    {t("scenes.inventoryScene.weight.current")}
                  </p>
                  <button
                    onClick={() => {
                      if (autoWeight)
                    ...
                    title={
                      autoWeight
                        ? t("scenes.inventoryScene.weight.enterManual")
                        : t("scenes.inventoryScene.weight.backToAuto")
                    }
                  ...
                  >
                    {autoWeight ? t("scenes.inventoryScene.weight.manual") : t("scenes.inventoryScene.weight.auto")}
                  </button>
                </div>
                {autoWeight ? (
                  <p
              ...
              <div className="text-center">
                <p className="text-[10px] uppercase tracking-widest text-amber-600 mb-0.5">
                  {t("scenes.inventoryScene.weight.max")}
                </p>
                <div className="flex items-baseline gap-1 justify-center">
                  <input
                    type="number"
                ...
                {mountCapacity > 0 && (
                  <p className="text-[10px] text-amber-500/80 mt-0.5">
                    {t("scenes.inventoryScene.weight.mountBonus", { value: mountCapacity })}{" "}
                    <span className="text-amber-300">
                      {t("scenes.inventoryScene.weight.totalWithMount", { value: effectiveMax })}
                    </span>
                  </p>
                )}
              </div>
              {effectiveMax > 0 && (
                <div className="flex-1 min-w-[120px]">
                  ...
                  >
                    {overEncumbered
                      ? t("scenes.inventoryScene.weight.overloaded")
                      : t("scenes.inventoryScene.weight.loadedPercent", { value: Math.round(weightPct) })}
                  </p>
                </div>
              )}
            </div>
          ...
          <section className="rounded-xl border border-amber-800/30 bg-[#2a1204]/70 p-5">
            <h2 className="text-xl font-semibold text-amber-300 mb-4 flex items-center gap-2">
              <Shirt className="w-5 h-5" /> {t("scenes.inventoryScene.equipment.title")}
            </h2>
            <div className="relative mx-auto" style={{ width: 480, height: 600 }}>
              <img
                src="/bg-inventary.png"
                alt={t("scenes.inventoryScene.common.characterAlt")}
            ...
            <p className="text-xs text-amber-700/50 text-center mt-3">
              {t("scenes.inventoryScene.equipment.hint")}
            </p>
          </section>
          {openSlot &&
            (() => {
              ...
              return (
                <SlotPickerModal
                  slotLabel={t(`scenes.inventoryScene.slots.${slot.key}`)}
                  slotKey={slot.key}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <section className="rounded-xl border border-amber-800/30 bg-[#2a1204]/70 p-5">
              <h2 className="text-xl font-semibold text-amber-300 mb-1 flex items-center gap-2">
                <FlaskConical className="w-5 h-5" /> {t("scenes.inventoryScene.sections.potions")}
              </h2>
              <p className="text-xs text-amber-600/70 mb-3 italic">
                {t("scenes.inventoryScene.sections.potionsDesc")}
              </p>
              <ConsumableSection
                ...
                inputPlaceholder={t("scenes.inventoryScene.sections.searchPotions")}
            ...
            <section className="rounded-xl border border-amber-800/30 bg-[#2a1204]/70 p-5">
              <h2 className="text-xl font-semibold text-amber-300 mb-1 flex items-center gap-2">
                <ScrollText className="w-5 h-5" /> {t("scenes.inventoryScene.sections.scrolls")}
              </h2>
              <p className="text-xs text-amber-600/70 mb-3 italic">
                {t("scenes.inventoryScene.sections.scrollsDesc")}
              </p>
              <ConsumableSection
                ...
                inputPlaceholder={t("scenes.inventoryScene.sections.searchScrolls")}
                onSelect={(name, weight, srdIndex, tags) =>
          ...
          <section className="rounded-xl border border-amber-800/30 bg-[#2a1204]/70 p-5">
            <div className="flex items-baseline gap-3 mb-1">
              <h2 className="text-xl font-semibold text-amber-300 flex items-center gap-2">
                <Package className="w-5 h-5" /> {t("scenes.inventoryScene.sections.bag")}
              </h2>
              <span className="text-xs text-amber-600">
                {t("scenes.inventoryScene.sections.objectsCount", { count: inventory.bag.reduce((s, i) => s + i.quantity, 0) })}
              </span>
            </div>
            <p className="text-xs text-amber-700/60 mb-3 italic">
              {t("scenes.inventoryScene.sections.bagDesc")}
            </p>
            <BagSection
              items={inventory.bag}
              allItems={compendiumItems}
          ...
          <section className="rounded-xl border border-amber-800/30 bg-[#2a1204]/70 p-5">
            <h2 className="text-xl font-semibold text-amber-300 mb-1 flex items-center gap-2">
              <Target className="w-5 h-5" /> {t("scenes.inventoryScene.sections.ammo")}
            </h2>
            <p className="text-xs text-amber-600/70 mb-3 italic">
              {t("scenes.inventoryScene.sections.ammoDesc")}
            </p>
            <ConsumableSection
              ...
              inputPlaceholder={t("scenes.inventoryScene.sections.searchAmmo")}
              onSelect={(name, weight, srdIndex, tags) =>
          ...
          <section className="rounded-xl border border-amber-800/30 bg-[#2a1204]/70 p-5 mx-auto w-full max-w-md">
            <h2 className="text-xl font-semibold text-amber-300 mb-4 flex items-center gap-2">
              <Coins className="w-5 h-5" /> {t("scenes.inventoryScene.sections.wallet")}
            </h2>
            <div className="grid grid-cols-5 gap-2">
              {CURRENCY_LABELS.map(({ key, color }) => (
                ...
                >
                  ...
                  >
                    {t(`scenes.inventoryScene.currency.${key}`)}
                  </span>
                  <input
                    type="number"
                    min={0}
              ...
              <p className="text-sm text-amber-600 flex items-center gap-2">
                ...
                <span>
                  <Link
                    ...
                    className="underline hover:text-amber-400"
                  >
                    {t("scenes.inventoryScene.footer.createAccount")}
                  </Link>{" "}
                  {t("scenes.inventoryScene.footer.createAccountSuffix")}
                </span>
              </p>
            ) : !linkedCharId ? (
              <p className="text-sm text-amber-600">
                {t("scenes.inventoryScene.footer.linkCharacterHint")}
              </p>
            ) : saveStatus === "saved" ? (
              <p className="text-sm text-green-400 flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4" /> {t("scenes.inventoryScene.footer.savedIn")} {" "}
                <strong>{linkedCharName}</strong>.
              </p>
            ) : saveStatus === "error" ? (
              <p className="text-sm text-red-400 flex items-center gap-2">
                <AlertCircle className="w-4 h-4" /> {t("scenes.inventoryScene.footer.error")}
              </p>
            ) : (
              <p className="text-sm text-amber-700">
                {t("scenes.inventoryScene.footer.unsavedHint")}
              </p>
            )}
            <Button
              onClick={handleSave}

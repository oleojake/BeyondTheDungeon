...
const EQUIPMENT_SLOTS = [
	{ key: "helmet", label: "Casco", icon: "⛑️", col: 2, row: 1 },
	{ key: "amulet", label: "Colgante", icon: "💎", col: 1, row: 2 },
	{ key: "armor", label: "Armadura", icon: "🛡️", col: 2, row: 2 },
	{ key: "cloak", label: "Capa", icon: "🧥", col: 3, row: 2 },
	{ key: "gloves", label: "Guantes", icon: "🧤", col: 1, row: 3 },
	{ key: "mainhand", label: "Arma principal", icon: "⚔️", col: 2, row: 3 },
	{ key: "offhand", label: "Arma sec.", icon: "🗡️", col: 3, row: 3 },
	{ key: "ring1", label: "Anillo izq.", icon: "💍", col: 1, row: 4 },
	{ key: "belt", label: "Cinturón", icon: "⭕", col: 2, row: 4 },
	{ key: "ring2", label: "Anillo der.", icon: "💍", col: 3, row: 4 },
	{ key: "boots", label: "Botas", icon: "👢", col: 2, row: 5 },
	{ key: "mount", label: "Montura", icon: "🐴", col: 3, row: 5 },
];
	...
	try {
		...
		return (
			<div className="text-xs text-gray-400 italic p-4 bg-gray-900/30 rounded border border-gray-700">
				No es un inventario estructurado. Visuarlización no disponible.
			</div>
		);
	}
	...
	return (
		<div className="space-y-4">
			...
			<div className="bg-gray-800/30 rounded-lg border border-gray-700 p-4">
				<h3 className="text-sm font-semibold text-amber-300 mb-3">⚔️ Equipo Equipado</h3>
					...
					{EQUIPMENT_SLOTS.map((slot) => (
						...
						>
							<div className="flex items-center gap-2">
								...
								<span className="text-xs text-gray-400 min-w-24">{slot.label}</span>
							</div>
							<span className="text-xs text-amber-300 font-semibold truncate">
								{parsed.equipped[slot.key]?.name || "—"}
							</span>
						</div>
					))}
				</div>
				...
				{totalPotions > 0 && (
					<div className="bg-gray-800/30 rounded-lg border border-gray-700 p-3">
						<h3 className="text-xs font-semibold text-red-300 mb-2 flex items-center gap-2">
							<FlaskConical className="w-3 h-3" /> Pociones ({totalPotions})
						</h3>
				...
				{totalScrolls > 0 && (
					<div className="bg-gray-800/30 rounded-lg border border-gray-700 p-3">
						<h3 className="text-xs font-semibold text-blue-300 mb-2 flex items-center gap-2">
							<ScrollText className="w-3 h-3" /> Pergaminos ({totalScrolls})
						</h3>
						<div className="space-y-1 max-h-32 overflow-y-auto">
				...
				{totalAmmo > 0 && (
					<div className="bg-gray-800/30 rounded-lg border border-gray-700 p-3">
						<h3 className="text-xs font-semibold text-green-300 mb-2 flex items-center gap-2">
							<Target className="w-3 h-3" /> Munición ({totalAmmo})
						</h3>
						<div className="space-y-1 max-h-32 overflow-y-auto">
			...
			{totalBagItems > 0 && (
				<div className="bg-gray-800/30 rounded-lg border border-gray-700 p-3">
					<h3 className="text-xs font-semibold text-amber-300 mb-2 flex items-center gap-2">
						<Package className="w-3 h-3" /> Inventario ({totalBagItems} objetos)
					</h3>
					<div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto">
						{parsed.bag.map((item) => (
							...
							>
								...
								{item.weight && (
									<span className="text-gray-500 text-xs">⚖️ {item.weight} lb</span>
								)}
							</div>
						))}
					</div>
			...
			<div className="bg-gray-800/30 rounded-lg border border-gray-700 p-3">
				...
				<div className="grid grid-cols-5 gap-2 text-center">
					...
					<div>
						...
						<div className="text-xs text-gray-500">Platino</div>
					</div>
				</div>
			</div>
		</div>
